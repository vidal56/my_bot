...
class ScanRelay(Node):
    def __init__(self):
        ...
        qos_sub = QoSProfile(depth=10, reliability=ReliabilityPolicy.BEST_EFFORT, durability=DurabilityPolicy.VOLATILE)
        qos_pub = QoSProfile(depth=10, reliability=ReliabilityPolicy.RELIABLE, durability=DurabilityPolicy.VOLATILE)
    ...
    def callback(self, msg):
        ...
        # Publica o feixe como linha
        if msg.ranges:
            dist = msg.ranges[len(msg.ranges)//2]
            if dist > 0.3 and dist < 12.0:
                marker = Marker()
